...
- **reencode_av1/** — Python package equivalent of reencode-av1.ps1, offering the same AV1 batch re-encoding with additional features. Run via `python -m reencode_av1 [directory]`. Requires ffmpeg and ffprobe on PATH. No external Python dependencies (standard library only). See `python -m reencode_av1 --help` for full usage. Key improvements over the PowerShell script: multi-segment sampling (encodes 5 segments via ffmpeg concat filter for more representative bitrate estimation), three selectable CRF search methods via `--search-method {smart,interpolation,binary}` (smart search by default — see `search.py` below; pure binary search; log-linear CRF interpolation), precise mode (`--precise`, redoes search with full-video encodes if the final bitrate is out of range; the pre-search full encode is preserved as a reusable seed so no re-encode is needed if the search settles on that same CRF), configurable audio bitrate (auto 64k/channel or manual override), and optional directory argument (defaults to the current directory).
