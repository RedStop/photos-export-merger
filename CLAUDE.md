...
- **reencode_av1/** — Python package equivalent of reencode-av1.ps1, offering the same AV1 batch re-encoding with additional features. Run via `python -m reencode_av1 [path]`, where `path` is a single video file or a directory to scan recursively (defaults to the current directory). Requires ffmpeg and ffprobe on PATH. No external Python dependencies (standard library only). See `python -m reencode_av1 --help` for full usage. Key improvements over the PowerShell script: multi-segment sampling (encodes 5 segments via ffmpeg concat filter for more representative bitrate estimation), three selectable CRF search methods via `--search-method {smart,interpolation,binary}` (smart search by default — see `search.py` below; pure binary search; log-linear CRF interpolation), precise mode (on by default; redoes search with full-video encodes if the final bitrate is out of range, with the pre-search full encode preserved as a reusable seed so no re-encode is needed if the search settles on that same CRF; disable with `--no-precise` to keep the first sample-based encode for speed, or use `--precise-only` to run the CRF search on the full video from the start regardless of length — the most accurate and slowest mode, with no sample pass or re-search, mutually exclusive with `--no-precise`), configurable audio bitrate (auto 64k/channel or manual override), optional positional path argument accepting a file or directory (defaults to the current directory), local scratch staging via `--scratch-dir` (copies each source to a local directory before encoding so the many decode passes read from local disk instead of a remote/NAS source, then moves the finished output to its destination; staging happens only after a file passes the skip checks, and the local copies are registered in the per-file `temp_files` cleanup list), fixed-CRF mode (set `--crf-min` equal to `--crf-max` to pin a CRF: the entire CRF search is skipped and every video is encoded once at that CRF regardless of bitrate; the `--target-bitrate`, window, `--search-method`, precise, and `--crf-ceiling-fallback` options are ignored — `validate_args` therefore relaxes to `0 <= crf-min <= crf-max <= 63` and drops the ceiling-fallback range check, and the source-bitrate/already-AV1/existing-output skips still apply; a one-time warning is logged at startup), and per-folder progress tracking (see `progress.py` below).

  Per-folder progress tracking: every folder that contains videos gets a `reencode-av1-progress.json` file recording the outcome of each video that has been processed or skipped, plus the final CRF for encoded videos. On a later run, videos already listed in their folder's progress file are dropped before processing — never probed, logged, or re-encoded — so no work is repeated. One line is logged per progress file consulted, reporting its path and how many of its videos were previously skipped vs previously re-encoded. The progress file is always read from and written to the real folder; it is never staged via `--scratch-dir`, so it stays in sync with the source tree. `process_file` returns a `FileResult` (status string + final CRF); the main loop records every outcome except failures (so they are retried) and dry-run previews (so a real run still processes them). `--no-progress` disables the whole mechanism: progress files are neither read (every video is re-examined) nor written (the run records nothing).

  Package structure:
  - `__main__.py` — CLI parsing, validation, and main processing loop (defines the `FileResult` dataclass returned by `process_file`)
  ...
  - `progress.py` — Per-folder progress tracking (`load_progress`, `record_progress`, `progress_path_for`, `PROGRESS_FILENAME`)
  - `search.py` — Outer CRF search loop with pluggable stateless search methods (`smart_search_next` — the default, `binary_search_next`, `interpolation_next`). Measured CRF/bitrate pairs are stored as `CrfPoint` dataclass instances (fields `crf`, `bitrate`) throughout — history, `seed_known`, and the search methods — so it is always clear which quantity is which (they move in opposite directions). The outer loop (`find_optimal_crf`) drives each iteration: asks the search method for the next CRF, nudges ±1 if the candidate is already in the history (direction chosen to push the bitrate toward the target), runs the encode, then checks the accept/confident window, the `--crf-max` ceiling (returns `crf-ceiling-fallback` with `crf_ceiling_used=True`), and the `--crf-min` floor. Accepts `seed_known` (pre-measured `CrfPoint` points) and `seed_temp_files` (a `crf -> Path` map of already-encoded files for those points); seeded files share the same temp-file lifecycle as in-loop encodes, so the winner's file is returned and non-winners are cleaned up. All search methods share one signature: `(history, crf_min, crf_max, accept_lo, accept_hi, seed_crf, confident_lo, confident_hi)`; the trailing confident-window bounds are passed by the outer loop but used only by `smart_search_next`. **Smart search** assumes monotonicity (bitrate decreases as CRF increases) and aims at the centre of the confident window — `(confident_lo + confident_hi) // 2`, which is the same convergence zone the outer loop checks (`sample_confident_*` for segment sampling, `final_accept_*` for full-video encodes). It probes the midpoint of `[crf_min, crf_max]` first, then — like a binary search — jumps to `crf_max` if the probe overshoots (CRF too small) or `crf_min` if it undershoots (CRF too large) to establish a bracket, and thereafter log-linearly interpolates between the closest pair straddling the target (clamped to lie strictly between them), terminating when that pair is consecutive or a one-sided history has already probed the relevant bound.
...
Test categories: `_parse_fraction` edge cases, `get_video_info` with mocked ffprobe (metadata parsing, VFR detection, missing streams, bitrate fallbacks, probe failures), `get_video_bitrate` (stream bitrate, file-size fallback, error handling), `_get_scale_filter` (landscape/portrait/square at various resolutions), `build_extra_args` (scaling, GOP calculation), `compute_segment_offsets` (spacing, clamping, boundary conditions), `compute_windows` (default values, low-target clamping), `_base_encode_args` (codec flags, CRF, preset, audio), `_extract_vf_filter` (extraction and edge cases), `_parse_time_to_seconds` (ffmpeg time string parsing), `interpolate_crf` (interpolation, clamping, log-linear verification), `binary_search_next`, `interpolation_next`, and `smart_search_next` (stateless next-CRF picking from history, including consecutive-bracket termination; smart search additionally covers midpoint first probe, jump-to-bound bracketing, one-sided-history termination, and confident-window-centre targeting), `find_optimal_crf` outer-loop behaviour (confident-zone exit, `--crf-max` ceiling triggering `crf_ceiling_used`, `--crf-min` floor, seeded search, duplicate-CRF nudging, winner temp-file preservation, seed temp-file reuse when the seed is selected and cleanup when it is not, encode-failure handling, default smart method and interpolation method via `search_method=`), `get_output_path` (extension handling), `compute_audio_bitrate` (auto/override), `validate_args` (including the positional `path` accepting either a directory or a file, rejecting a non-existent path, rejecting `--precise-only` combined with `--no-precise`, `--scratch-dir` accepting a non-existent path but rejecting an existing file, and fixed-CRF mode accepting equal `--crf-min`/`--crf-max` plus an out-of-point `--crf-ceiling-fallback`), `build_parser` (CLI defaults and parsing, including `--search-method` choices and rejection of unknown methods, `precise` defaulting to True with `--no-precise` turning it off, `--precise-only` defaulting to False, `progress` defaulting to True with `--no-progress` turning it off, the positional `path` defaulting to None, and `--scratch-dir` defaulting to None), `process_file` (skip conditions, dry run, temp file cleanup on exception, `--precise-only` forcing a full-video search with no segment offsets on a long video and reusing the search encode without a separate full encode, `--scratch-dir` staging — source copied to the scratch dir, encode reads the staged copy, output moved to its destination on success, and no files left behind on failure, and fixed-CRF mode — equal `--crf-min`/`--crf-max` skips the search and encodes once at the pinned CRF even on a long video, an out-of-range output bitrate does not trigger the precise re-search, and the `--skip-below-bitrate` skip still fires), per-folder progress tracking (`load_progress`/`record_progress` round-trip, preserving existing entries across records, overwriting an entry for the same file, and returning an empty dict for missing/malformed/non-object progress files), and parametrized window invariant checks across multiple parameter combinations.
