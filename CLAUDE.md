...
- **reencode_av1/** — Python package equivalent of reencode-av1.ps1, offering the same AV1 batch re-encoding with additional features. Run via `python -m reencode_av1 [directory]`. Requires ffmpeg and ffprobe on PATH. No external Python dependencies (standard library only). See `python -m reencode_av1 --help` for full usage. Key improvements over the PowerShell script: multi-segment sampling (encodes 5 segments via ffmpeg concat filter for more representative bitrate estimation), log-linear CRF interpolation (`--interpolate`), precise mode (`--precise`, redoes search with full-video encodes if the final bitrate is out of range; the pre-search full encode is preserved as a reusable seed so no re-encode is needed if the search settles on that same CRF), configurable audio bitrate (auto 64k/channel or manual override), and optional directory argument (defaults to the current directory).
  ...
  - `search.py` — Outer CRF search loop with pluggable stateless search methods (`binary_search_next`, `interpolation_next`). Measured CRF/bitrate pairs are stored as `CrfPoint` dataclass instances (fields `crf`, `bitrate`) throughout — history, `seed_known`, and the search methods — so it is always clear which quantity is which (they move in opposite directions). The outer loop (`find_optimal_crf`) drives each iteration: asks the search method for the next CRF, nudges ±1 if the candidate is already in the history (direction chosen to push the bitrate toward the target), runs the encode, then checks the accept/confident window, the `--max-crf` ceiling (returns `crf-ceiling-fallback` with `crf_ceiling_used=True`), and the `--crf-min` floor. Accepts `seed_known` (pre-measured `CrfPoint` points) and `seed_temp_files` (a `crf -> Path` map of already-encoded files for those points); seeded files share the same temp-file lifecycle as in-loop encodes, so the winner's file is returned and non-winners are cleaned up.
...
Test categories: `_parse_fraction` edge cases, `get_video_info` with mocked ffprobe (metadata parsing, VFR detection, missing streams, bitrate fallbacks, probe failures), `get_video_bitrate` (stream bitrate, file-size fallback, error handling), `_get_scale_filter` (landscape/portrait/square at various resolutions), `build_extra_args` (scaling, GOP calculation), `compute_segment_offsets` (spacing, clamping, boundary conditions), `compute_windows` (default values, low-target clamping), `_base_encode_args` (codec flags, CRF, preset, audio), `_extract_vf_filter` (extraction and edge cases), `_parse_time_to_seconds` (ffmpeg time string parsing), `interpolate_crf` (interpolation, clamping, log-linear verification), `binary_search_next` and `interpolation_next` (stateless next-CRF picking from history, including consecutive-bracket termination), `find_optimal_crf` outer-loop behaviour (confident-zone exit, `--max-crf` ceiling triggering `crf_ceiling_used`, `--crf-min` floor, seeded search, duplicate-CRF nudging, winner temp-file preservation, seed temp-file reuse when the seed is selected and cleanup when it is not, encode-failure handling, interpolation method via `search_method=`), `get_output_path` (extension handling), `compute_audio_bitrate` (auto/override), `validate_args`, `build_parser` (CLI defaults and parsing), `process_file` (skip conditions, dry run, temp file cleanup on exception), and parametrized window invariant checks across multiple parameter combinations.
